...
def request_otp_view(request):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "✅ ส่งรหัส OTP ไปยังอีเมลของคุณแล้ว")
...
def verify_otp_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.error(request, "ไม่พบรหัส OTP กรุณาขอใหม่")
            return redirect("request_otp")

        if otp_obj.is_valid(code):
            request.session["otp_verified"] = True
            messages.success(request, "✅ ยืนยันรหัส OTP สำเร็จ โปรดตั้งรหัสผ่านใหม่")
...
def reset_password_custom(request):
    """ตั้งรหัสผ่านใหม่หลังจากผ่าน OTP"""
    ...
    if request.method == "POST":
        ...
        messages.success(request, "🎉 เปลี่ยนรหัสผ่านเรียบร้อยแล้ว สามารถเข้าสู่ระบบได้เลย")
        return redirect("login")

    return render(request, "otp/reset_password_custom.html")

class CustomPasswordResetView(PasswordResetView):
    """ใช้ domain/protocol จาก settings"""
